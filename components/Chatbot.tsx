...
const Chatbot: React.FC<ChatbotProps> = ({ onMeetingScheduled }) => {
    ...
    // Booking-detection regex (keeps your original logic)
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            // Send message to server-side Gemini proxy
            const response = await sendMessageToGemini(userMessage.content);

            const modelMessage: ChatMessage = { role: 'model', content: response };
            setMessages(prev => [...prev, modelMessage]);

            // Check for successful booking and trigger the modal (keeps your original behavior)
            ...
            const errMsg: ChatMessage = { role: 'model', content: "Sorry — something went wrong. Please try again later." };
